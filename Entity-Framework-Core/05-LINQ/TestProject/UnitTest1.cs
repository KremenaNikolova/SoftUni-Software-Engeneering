...
public class Export_003
{
    ...
    public void Setup()
    {
        ...
        this.serviceProvider = ConfigureServices<MusicHubDbContext>("MusicHub");
    }
    ...
    public void ExportGamesByGenresZeroTest()
    {
        ...
        var actual = StartUp.ExportAlbumsInfo(context, 4);

        var expected = @"-AlbumName: Barrage of noise
-ReleaseDate: 07/03/2018
-ProducerName: Evtim Miloshev
-Songs:
---#1
...
-AlbumPrice: 7.00";
        ...
        Assert.That(actual, Is.EqualTo(expected).NoClip,
            $"{nameof(StartUp.ExportAlbumsInfo)} output is incorrect!");
    }

    private static void SeedDatabase(MusicHubDbContext context)
    {
        var datasetsJson =
